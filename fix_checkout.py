...
new_block = """let orderHistoryUnsub = null;
...
content = re.sub(r"window\.openOrderHistory = async \(\) => \{.*?list\.innerHTML = '<p style=\"color:#ff6b6b;text-align:center;\">Error loading orders\.</p>';\s*\}\s*\};", new_block, content, flags=re.DOTALL)

with open('checkout.html', 'w', encoding='utf-8') as f:
    f.write(content)
print("checkout fixed")
